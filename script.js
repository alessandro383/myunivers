...
async function addItem() {
    ...
    // 🔎 catégories avec API (on OUVRE un sélecteur)
...
async function searchBook(name) {
    ...
    json.items.forEach((book, index) => {
        ...
        const div = document.createElement("div");
        div.className = "dvd-item";
        div.innerHTML = `<img src="${info.imageLinks?.thumbnail || 'https://via.placeholder.com/100'}" width="70"><p>${info.title}</p><button onclick="selectBookFromAPI(${index})">Choisir</button>`;
        list.appendChild(div);
    });
    document.getElementById("dvdModal").classList.remove("hidden");
}
...
async function selectBookFromAPI(index) {
    ...
    await supabaseClient.from("items").insert([{ user: currentUser, name: book.title, image: book.imageLinks?.thumbnail || "https://via.placeholder.com/150", category: "livre", status: "a_lire" }]);
    document.getElementById("dvdModal").classList.add("hidden");
    document.getElementById("name").value = "";
    render();
}
